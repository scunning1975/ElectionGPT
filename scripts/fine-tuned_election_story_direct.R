...
model_id <- "ft:gpt-4o-mini-2024-07-18:personal::A5g8HA2H" # Replace with your actual fine-tuned model ID

# Directory where stories and results will be saved
save_directory <- "/Users/jaredblack/GitHub/ElectionGPT/data"

# Define the prompt
prompt <- "Generate a short news story about the outcome of the 2024 US presidential election between Donald Trump and Kamala Harris. The story must explicitly list all 50 states and their winners in the format 'State: Winner'."
...
combined_story_filename <- paste0("story_Overall_", timestamp, ".txt")
...
for (i in 1:100) {
  ...
  if (!is.null(story)) {
    ...
    winners_df <- extract_winners(openai_api_key, story)
    
    if (!is.null(winners_df)) {
      # Attempt to create the results row, with error handling
      tryCatch({
        trial_results <- winners_df$Winner
        names(trial_results) <- winners_df$Abbreviation
        trial_results <- c(Trial = paste("Trial", i), trial_results)
...
results_filename <- paste0("election_results_Overall_", timestamp, ".xlsx")
results_filepath <- file.path(save_directory, "raw", results_filename)
