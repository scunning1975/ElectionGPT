...
openai_api_key <- Sys.getenv("OVERALL_MSNBC_KEY")
...
# Define the prompt
prompt <- "Given the recent news articles provided, generate a short news story from the perspective of MSNBC reporter Rachel Maddow about the outcome of the 2024 US presidential election between Donald Trump and Kamala Harris. The story must explicitly state the winner in each of the 50 states. Include a specific detail or quote pulled directly from the news articles."
...
test_story_filename <- paste0("story_overall_MSNBC", timestamp, ".txt")
...
# Number of trials
num_trials <- 1000  # Set to 2 for testing; change back to 100 for full run
...
for (i in 1:num_trials) {
  ...
  if (!is.null(story)) {
    ...
    if (!is.null(winners_csv)) {
      ...
      tryCatch({
        # Convert 'Winner' column to numeric values: 1 for Trump, 0 for Harris
        winners_csv$Winner <- as.numeric(winners_csv$Winner)
        
        # Initialize a vector for this trial's results with NA
        trial_vector <- rep(NA, length(state.abb))
        names(trial_vector) <- state.abb
        
        # Match states and fill in the winners
        # Ensure state names in winners_csv match state.abb
        winners_csv$State <- state.name[match(winners_csv$State, state.name)]
