...
openai_api_key <- Sys.getenv("GPT4O_MINI_BBC_API_KEY")
...
# Define the prompt
prompt <- "Given the recent news articles in the JSON data provided, generate a short news story from the perspective of BBC reporter Laura Kuenssberg about the outcome of the 2024 US presidential election between Donald Trump and Kamala Harris. The story must explicitly state the winner in each of the 50 states. Include a specifc detail or quote pulled directly from the news articles."
...
combined_story_filename <- paste0("story_BBC_", timestamp, ".txt")  # Adjust this part for each script (direct, Fox, MSNBC, BBC)
...
results_filename <- paste0("election_results_BBC_", timestamp, ".xlsx")
